...
function init!(model::TensorAutoregression)
    ...
    if isa(coef(model), StaticKruskal)
        ...
        cov(coef(model)).data .= cov(λ_lead - dynamics(coef(model)) * λ_lag, dims=2)
    end
...
absdiff(ε::TensorNormal, ε_prev::TensorNormal) = absdiff.(cov(ε), cov(ε_prev))
function absdiff(A::StaticKruskal, A_prev::StaticKruskal)
    δ_loadings = absdiff(loadings(A), loadings(A_prev))
    δ_factors = absdiff(factors(A), factors(A_prev))
...
function absdiff(A::DynamicKruskal, A_prev::DynamicKruskal)
    ...
    return max(δ_loadings, δ_factors, δ_dynamics, δ_cov)
end
